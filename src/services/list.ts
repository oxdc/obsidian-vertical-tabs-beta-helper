...
export async function listBuilds(token: string, limit: number, offset: number): Promise<ListBuildsResponse> {
	...
	return await retryWithBackoff(async () => await apiService.listBuilds(limit, offset), retryConfig);
}
